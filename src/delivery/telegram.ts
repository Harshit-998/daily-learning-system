...
export async function sendTelegramDocument(config: AppConfig, html: string, fileName: string): Promise<string> {
  ...
  form.append("document", new Blob([html], { type: "text/html; charset=utf-8" }), fileName);
  ...
  if (!response.ok) {
    throw new Error(`Telegram document send error ${response.status}: ${await response.text()}`);
  }
